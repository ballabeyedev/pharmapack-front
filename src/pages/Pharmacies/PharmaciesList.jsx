...
export default function PharmaciesPage() {
  ...
  const [vue,           setVue]           = useState('validees');
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      if (action === 'valider') {
        ...
        showToast(`Pharmacie « ${pharmacie.nom_pharmacie} » validée avec succès`);
      } else {
        await rejeterPharmacie(pharmacie.id);
        showToast(`Pharmacie « ${pharmacie.nom_pharmacie} » rejetée`, 'error');
      }
      setConfirm(null);
      load();
    } catch {
  ...
  const filtered = pharmacies.filter(p => {
    ...
    const matchSearch =
      ...
      (p.ville_pharmacie || '').toLowerCase().includes(q) ||
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', background: C.greenPale, borderRadius: '12px', padding: '4px', gap: '4px', flexShrink: 0 }}>
          {[
            ...
            <button key={key} onClick={() => setVue(key)}
              ...
              {key === 'en_attente' && nbAttente > 0 && !isAttente && (
                <span style={{ marginLeft: '6px', background: '#f59e0b', color: '#fff', borderRadius: '10px', padding: '1px 7px', fontSize: '0.72rem', fontWeight: 800 }}>
                  {nbAttente}
                </span>
              )}
            </button>
                  ...
                  ? (
                    <tr>
                      <td colSpan={isAttente ? 5 : 6} style={{ padding: '60px', textAlign: 'center', color: C.textMuted }}>
                        <div style={{ fontSize: '2.5rem', marginBottom: '10px' }}>{isAttente ? '⏳' : '🔍'}</div>
                    ...
                    return (
                      <tr key={p.id} style={{ borderBottom: `1px solid ${C.border}`, transition: 'background 0.15s', animation: `fadeUp 0.3s ease ${i * 0.04}s both` }}>
                        ...
                        <td style={td}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <div style={{ width: '38px', height: '38px', flexShrink: 0, background: `linear-gradient(135deg,${C.greenMid},${C.greenBright})`, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              {p.logo
                                ? <img src={p.logo} alt="logo" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '10px' }} />
                                : <span style={{ color: '#fff', fontWeight: 800, fontSize: '0.9rem' }}>{(p.nom_pharmacie || '?')[0].toUpperCase()}</span>
                              }
                            </div>
                            <div>
                        ...
                        <td style={td}>
                          ...
                          <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginTop: '2px' }}>{u.telephone || ''}</p>
                        </td>
                        ...
                        {/* Statut */}
                        {!isAttente && (
                          <td style={td}><StatutBadge statut={u.statut} /></td>
                        )}
                        ...
                        <td style={td}>
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'nowrap' }}>
                            ...
                            <button
                              onClick={() =>
                                navigate(`/pharmaPack/admin/pharmacies/${p.id}`, {
                                  state: { pharmacie: p }
                                })
                              }
