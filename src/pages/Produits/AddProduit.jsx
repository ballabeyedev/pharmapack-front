...
const inputStyle = (error) => ({
  ...
  color: C.textDark, background: '#fff', outline: 'none',
  fontFamily: 'inherit', boxSizing: 'border-box', transition: 'border-color 0.15s',
});

const Label = ({ children, required }) => (
  <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700,
    color: C.textMuted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>
    {children}
...
const Section = ({ title, icon, children }) => (
  <div style={{ background: '#fff', borderRadius: '16px', padding: '24px',
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px',
      ...
      <h3 style={{ fontFamily: 'Georgia,serif', fontSize: '0.95rem', fontWeight: 700, color: C.textDark }}>
        {title}
      </h3>
    </div>
    {children}
...
/* ─────────────────────────────────────────────
   Page AddProduit (avec toggle Import)
───────────────────────────────────────────── */
export default function AddProduit({ onBack, onSuccess }) {
  ...
  const [form, setForm] = useState({
    ...
    stock: '', image: '', imageFile: null,
    dimension: '', unite: '', statut: true, categorie_id: '',
  });
    ...
    try {
      ...
      if (form.imageFile) {
        ...
        payload = {
          ...
          image: null, dimension: form.dimension.trim() || null,
          unite: form.unite || null, statut: form.statut,
  ...
  const removeImage = () => { setForm(f => ({ ...f, imageFile: null, image: '' })); setPreview(null); };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={onBack}
            style={{ display: 'flex', alignItems: 'center', gap: '6px',
              ...
              fontSize: '0.84rem', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>
        ...
        {/* ── Bouton bascule Import ── */}
        <button
          onClick={() => setMode('import')}
          style={{
            display: 'flex', alignItems: 'center', gap: '8px',
            padding: '10px 18px', border: `1.5px solid ${C.greenBorder}`,
            borderRadius: '12px', background: C.greenPale, color: C.greenDeep,
            fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer',
            fontFamily: 'inherit', transition: 'all 0.15s',
            boxShadow: '0 2px 8px rgba(13,92,58,0.1)',
          }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span style={{ fontSize: '0.75rem', color: C.textMuted, fontWeight: 600,
          padding: '4px 12px', background: '#f3f4f6', borderRadius: '20px' }}>
          OU SAISIE MANUELLE
        </span>
        <div style={{ flex: 1, height: '1px', background: C.border }} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          ...
          <Section title="Informations générales"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <textarea style={{ ...inputStyle(false), minHeight: '90px', resize: 'vertical' }}
              ...
              <div>
                ...
                <select style={{ ...inputStyle(false), cursor: 'pointer' }}
                  value={form.categorie_id} onChange={e => set('categorie_id', e.target.value)}>
              ...
              <div>
                ...
                <select style={{ ...inputStyle(false), cursor: 'pointer' }}
                  value={form.unite} onChange={e => set('unite', e.target.value)}>
                  <option value="">— Sélectionner —</option>
        ...
        <Section title="Prix"
          ...
          <input style={inputStyle(errors.prix)} type="number" min="0" placeholder="Ex: 2500"
          ...
          {form.prix && form.prix_promo && Number(form.prix_promo) < Number(form.prix) && (
            <div style={{ padding: '10px 14px', background: C.greenPale,
              ...
              <p style={{ fontSize: '0.8rem', color: C.greenDeep, fontWeight: 600 }}>
                💰 Remise de{' '}
                <strong>{Math.round((1 - Number(form.prix_promo) / Number(form.prix)) * 100)}%</strong>
                {' '}sur le prix normal
              </p>
            </div>
          )}
        ...
        <Section title="Stock"
          ...
          <input style={inputStyle(errors.stock)} type="number" min="0" placeholder="Ex: 1500"
            ...
            <div style={{ marginTop: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                ...
                <span style={{ fontSize: '0.75rem', fontWeight: 700,
                  color: Number(form.stock) < 500 ? '#d97706' : Number(form.stock) < 2000 ? C.greenMid : C.greenDeep }}>
                  {Number(form.stock) < 500 ? '⚠ Faible' : Number(form.stock) < 2000 ? 'Moyen' : 'Bon'}
                </span>
              </div>
              ...
              <div>
                ...
                <label htmlFor="image-upload"
                  style={{
                    ...
                    justifyContent: 'center', gap: '10px', padding: '28px 20px',
                    border: `2px dashed ${preview ? C.greenMid : C.border}`,
                    borderRadius: '14px', background: preview ? C.greenPale : '#fafafa',
                    cursor: 'pointer', transition: 'all 0.2s', textAlign: 'center',
                  }}>
                  {preview ? (
                    <>
                      <Icon d="M5 13l4 4L19 7" size={22} stroke={C.greenMid} />
                      <p style={{ fontSize: '0.85rem', fontWeight: 700, color: C.greenDeep }}>{form.image}</p>
                      <p style={{ fontSize: '0.75rem', color: C.textMuted }}>Cliquez pour changer l'image</p>
                    </>
                  ) : (
                    <>
                      ...
                      <div>
                        <p style={{ fontSize: '0.88rem', fontWeight: 700, color: C.textDark }}>Choisir une image</p>
                        <p style={{ fontSize: '0.75rem', color: C.textMuted, marginTop: '2px' }}>PNG, JPG, WEBP · max 5 Mo</p>
                      </div>
                    </>
                  )}
                </label>
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end',
        ...
        <button onClick={handleSubmit} disabled={loading}
          style={{ padding: '12px 28px', border: 'none', borderRadius: '12px',
            background: loading ? '#9ca3af' : `linear-gradient(135deg,${C.greenDeep},${C.greenMid})`,
            color: '#fff', fontWeight: 700, fontSize: '0.9rem',
            cursor: loading ? 'not-allowed' : 'pointer',
            fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '8px',
            boxShadow: loading ? 'none' : '0 4px 14px rgba(13,92,58,0.3)', transition: 'all 0.15s' }}>
          {loading ? (
            <>
