...
import { modifierProduit, getCategories } from '../../services/admin.service';
...
/* ─────────────────────────────────────────────
   Page EditProduit
───────────────────────────────────────────── */
export default function EditProduit({ produit, onBack, onSuccess }) {
  const [categories, setCategories] = useState([]);
  const [loading,    setLoading]    = useState(false);
  const [preview,    setPreview]    = useState(produit?.image || null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await modifierProduit(produit.id, payload);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
        ...
        <div>
          ...
          <p style={{ color: C.textMuted, fontSize: '0.8rem' }}>
            Modifiez les informations de <strong>{produit?.nom}</strong>
          </p>
        </div>
      </div>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          <Section title="Image du produit"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '20px', alignItems: 'flex-start' }}>
              <div>
                <input id="image-upload-edit" type="file" accept="image/*"
                  ...
                  {preview ? (
                    <>
                      ...
                      <p style={{ fontSize: '0.85rem', fontWeight: 700, color: C.greenDeep }}>
                        {form.imageFile ? form.image : 'Image actuelle conservée'}
                      </p>
                ...
                {preview && (
                  <button onClick={removeImage}
                    style={{
                      marginTop: '10px', padding: '7px 14px', border: 'none',
      ...
      <div style={{
        ...
        <button onClick={onBack}
          style={{
            padding: '12px 24px', border: `1.5px solid ${C.border}`,
