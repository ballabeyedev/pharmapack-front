...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'conversions', label: 'Conversions', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
      ...
      { key: 'admins', label: 'Administrateurs', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
    ]
  },
  {
    group: 'Compte',
...
export default function Sidebar({ page, setPage, open, mobile, mobileSide, setMobileSide, onLogout }) {
  ...
  return (
    <div style={{
      ...
      <nav style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '12px 0' }}>
        {NAV_ITEMS.map(group => (
          <div key={group.group}>
            {isVisible && (
              <p style={{
                padding: '10px 20px 4px', fontSize: '0.65rem', fontWeight: 700,
            ...
            {group.items.map(item => {
              ...
              return (
                <button key={item.key} onClick={() => handleNav(item.key)}
                  ...
                  style={{
                    width: '100%', padding: isVisible ? '10px 20px' : '10px 0',
      ...
      {/* Logout */}
      <div style={{
        padding: isVisible ? '12px 12px 20px' : '12px 0 20px',
        borderTop: '1px solid rgba(255,255,255,0.12)', flexShrink: 0
      }}>
        <button onClick={onLogout}
          style={{
            width: '100%', padding: isVisible ? '10px 16px' : '10px 0',
